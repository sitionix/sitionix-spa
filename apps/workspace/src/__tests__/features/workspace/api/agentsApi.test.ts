...
import { AgentApi, AgentChatApi, AgentConversationApi, AgentRuleApi } from "@sitionix/app-afesox-bffssox-frontend-sitionix-140-unstable/apis";
...
describe("agentsApi.projects", () => {
  ...
  it("patches project context when context is explicitly null", async () => {
    const requestJsonSpy = vi.spyOn(httpClient, "requestJson").mockResolvedValue({
      ok: true,
      data: {
        id: "project-1",
    ...
    await patchAgentProject("project-1", { context: null });
