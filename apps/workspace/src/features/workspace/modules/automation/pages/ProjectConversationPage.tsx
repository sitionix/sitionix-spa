...
const USER_MESSAGE_CLASS = "max-w-[80%] rounded-2xl bg-zinc-900 px-4 py-2 text-sm text-white";
const AGENT_MESSAGE_CLASS = "max-w-[80%] rounded-2xl bg-zinc-100 px-4 py-2 text-sm text-zinc-900";
...
function MessageList({ messages, isAwaitingExecution }: { messages: ChatAgentMessage[]; isAwaitingExecution: boolean }) {
    ...
    return (
      <div className="flex h-full min-h-[420px] items-center justify-center">
        <div className="max-w-md text-center">
          ...
          <p className="mt-2 text-sm text-zinc-600">Start the conversation with your first message.</p>
        </div>
      </div>
    );
  }
      ...
      {messages.map((message) => {
        ...
        return (
          <div key={message.id} className={isUser ? "flex justify-end" : "flex justify-start"}>
            <div className={isUser ? USER_MESSAGE_CLASS : AGENT_MESSAGE_CLASS}>{message.content}</div>
          </div>
        );
      })}
...
function ParticipantsList({ participants }: { participants: ProjectConversationParticipant[] }) {
  return (
    <ul className="mt-3 space-y-2">
      {participants.map((participant) => {
        ...
        const description = participant.description?.trim() ? participant.description : "No description yet.";
...
export function ProjectConversationPage() {
  ...
  const participants = useMemo(
    () => (conversation?.participants ?? []).filter((participant) => participant.type === "AGENT"),
    ...
    <>
      ...
      {status === "not_found" ? (
        <StatusBlock
          ...
          titleClassName="text-xl font-semibold text-zinc-900"
      ...
      {status === "error" ? (
        <StatusBlock
          ...
          titleClassName="text-lg font-semibold text-red-900"
          descriptionClassName="mt-2 text-sm text-red-700"
        />
      ) : null}
          ...
          <section className="space-y-4">
            <header className="rounded-3xl border border-zinc-200 bg-white p-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-zinc-600">Project conversation</p>
                </div>
              </div>
            </header>
            ...
            <section className="flex min-h-[560px] flex-col rounded-3xl border border-zinc-200 bg-white">
              <div className="flex-1 space-y-3 overflow-y-auto px-5 py-5">
                <MessageList messages={messages} isAwaitingExecution={isAwaitingExecution} />
              </div>
